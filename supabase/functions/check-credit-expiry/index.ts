  ...
  try {
    ...
    for (const credit of expiringCredits) {
      try {
        ...
        if (customer?.phone) {
          try {
            const smsMessage = `Novara reminder: You have ${credit.credits_remaining} cleaning credit${credit.credits_remaining > 1 ? 's' : ''} expiring on ${expiryDate}. Book now: https://try.novaracleaning.com/book/zip`;
