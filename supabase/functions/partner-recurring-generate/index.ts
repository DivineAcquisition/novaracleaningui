...
// The date in the Monday-anchored week whose day-of-month matches `dom`
// (clamped to the month's length), or null when this month's occurrence
// doesn't fall inside the week.
function monthlyDateInWeek(weekStartMonday: string, dom: number): string | null {
  for (let i = 0; i < 7; i++) {
    const iso = addDays(weekStartMonday, i);
    const d = new Date(`${iso}T00:00:00Z`);
    const lastDay = new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth() + 1, 0)).getUTCDate();
    if (d.getUTCDate() === Math.min(dom, lastDay)) return iso;
  }
  return null;
}
...
serve(async (req) => {
  ...
  for (const sch of list) {
    const frequency = (sch.frequency as string) || "weekly";
    ...
    // Resolve the date(s) this schedule produces in the upcoming week.
    // weekly  → one per selected day-of-week.
    // monthly → the day_of_month occurrence IF it falls inside this week.
    let dates: string[] = [];
    let monthKey = "";
    if (frequency === "monthly") {
      const dom = Number(sch.day_of_month) || 0;
      const occ = dom > 0 ? monthlyDateInWeek(nextWeek, dom) : null;
      if (!occ) { results.push({ id: sch.id, skipped: "monthly_not_in_week" }); continue; }
      dates = [occ];
      monthKey = occ.slice(0, 7); // YYYY-MM
    } else {
      const days = (sch.days_of_week as number[]) || [];
      if (days.length === 0) { results.push({ id: sch.id, skipped: "no_days" }); continue; }
      dates = days.map((d) => dateForDow(nextWeek, d));
    }
    const total = price * dates.length;
    const periodLabel = frequency === "monthly"
      ? formatServiceDate(dates[0])
      : `week of ${formatServiceDate(nextWeek)}`;

    if (mode === "heads_up") {
      const already = frequency === "monthly"
        ? String(sch.last_generated_month || "") === monthKey || (sch.heads_up_week as string) === `m:${monthKey}`
    ...
    // generate — idempotency guard per cadence period.
    const alreadyGenerated = frequency === "monthly"
      ? String(sch.last_generated_month || "") === monthKey
      : String(sch.last_generated_week || "") === nextWeek;
    if (alreadyGenerated) { results.push({ id: sch.id, skipped: "already_generated" }); continue; }
    // Claim the period immediately to guarantee idempotency (no double-charge).
    await admin.from("recurring_schedules").update(
      frequency === "monthly"
        ? { last_generated_month: monthKey, last_generated_week: nextWeek }
        : { last_generated_week: nextWeek },
    ).eq("id", sch.id);
