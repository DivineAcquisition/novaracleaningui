...
serve(async (req) => {
  ...
  try {
    ...
    // Resolve via DB-override layer so a Stripe key rotated through
    // `public.app_secrets` takes effect on next cold start without
    // requiring a dashboard env var change.
