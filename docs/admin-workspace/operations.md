...
Screening-Passed or later, not yet Active, never rejected, and have a valid photo ID and
own vehicle. They must be within the max-miles window (45–55 miles, default 45). The offer
pays the first-job share (default 45%) plus 70¢/mi mileage in one total, with company take
floored at 40% of job value. The message acknowledges they are receiving it because they
applied in the past. Recipients still need a valid supply checklist, a signed agreement, and
payout setup before they can accept — background check is **not** required on this path. First
to finish those steps and accept wins; everyone else sees the job as no longer available and
keeps any onboarding progress. If the fill window closes with nobody accepting, it surfaces
here as still needing coverage, with a link to the **Urgent Hire** log on Cleaners
(`/admin/cleaners?section=urgent-hire`).
