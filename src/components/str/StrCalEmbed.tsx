...
// Cal.com 15-minute discovery calendar for unusual STR listings.
// Same event the rest of intake already uses: malik-sannie-clwphb/15min.

import { useEffect, useId, useRef } from "react";
import { STR_CAL_LINK, STR_CAL_NAMESPACE, STR_CAL_ORIGIN } from "@/lib/host-landing/landing";
...
function ensureCal(): void {
  ...
  const d = C.document;
  C.Cal = C.Cal || (function calBootstrap(this: unknown) {
...
export function StrCalEmbed({
  ...
  const elementId = `str-cal-${reactId}`;
  ...
  useEffect(() => {
    ...
    Cal("init", STR_CAL_NAMESPACE, { origin: STR_CAL_ORIGIN });
    const ns = Cal.ns?.[STR_CAL_NAMESPACE];
    if (!ns) return;
