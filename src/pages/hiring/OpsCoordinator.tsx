...
const jobData = {
  ...
  benefits: [
    ...
    { icon: Clock, title: 'Flexible Hours', desc: 'Core hours with flexibility' },
    { icon: TrendingUp, title: 'Growth Path', desc: 'Advance as we scale' },
    { icon: HeartHandshake, title: 'Health Stipend', desc: 'Insurance contribution' },
    { icon: Calendar, title: 'PTO', desc: 'Paid time off + holidays' },
    { icon: Settings, title: 'Equipment', desc: 'Home office allowance' },
  ],
};
...
const responsibilityIcons = [
  ...
  <Target key="2" className="w-4 h-4" />,
  <MessageSquare key="3" className="w-4 h-4" />,
  <BarChart key="4" className="w-4 h-4" />,
  <ClipboardList key="5" className="w-4 h-4" />,
  <Settings key="6" className="w-4 h-4" />,
  <Users key="7" className="w-4 h-4" />,
];
...
export default function OpsCoordinatorRole() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsApplying(false);
    setFormData({ fullName: '', email: '', phone: '', linkedin: '', experience: '', whyYou: '', availability: '' });
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      placeholder="Describe your experience with operations, scheduling, or coordination..."
                    />
                  </div>
