...
interface Recurring { id: string; property_id: string; frequency?: string | null; days_of_week: number[]; day_of_month?: number | null; window_start: string | null; window_end: string | null; active: boolean; paused_until: string | null; }
...
export default function PartnerWeeklySchedule() {
  ...
  // Recurring-plan creator (weekly OR monthly).
  const [recProp, setRecProp] = useState("");
  const [recFreq, setRecFreq] = useState<"weekly" | "monthly">("weekly");
  const [recDows, setRecDows] = useState<number[]>([]);
  const [recDom, setRecDom] = useState("1");
  const [recWs, setRecWs] = useState("11:00");
  const [recWe, setRecWe] = useState("15:00");
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: props }, { data: rec }, { data: hostRow }] = await Promise.all([
      ...
      (supabase.from as any)("hosts").select("calendar_token").limit(1).maybeSingle(),
    ]);
    setProperties((props as Property[]) || []);
    setRecurring((rec as Recurring[]) || []);
    setCalendarToken((hostRow as { calendar_token?: string } | null)?.calendar_token || null);
    setLoading(false);
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const saveRecurring = async () => {
    if (!recProp) { toast.error("Pick a property."); return; }
    if (recFreq === "weekly" && recDows.length === 0) { toast.error("Pick at least one day."); return; }
    if (minutes(recWs) >= minutes(recWe)) { toast.error("Checkout time must be before the check-in deadline."); return; }
    setRecSaving(true);
    const { data, error } = await supabase.functions.invoke("partner-turnover", {
      body: {
        action: "recurring.save",
        propertyId: recProp,
        frequency: recFreq,
        days_of_week: recFreq === "weekly" ? recDows : [],
        day_of_month: recFreq === "monthly" ? parseInt(recDom, 10) : undefined,
        window_start: recWs,
        window_end: recWe,
      },
    });
    setRecSaving(false);
    if (error || (data as any)?.error) { toast.error((data as any)?.error || "Could not save plan"); return; }
    toast.success("Recurring plan saved — we'll auto-schedule + charge per clean.");
    setRecProp(""); setRecDows([]); setRecDom("1"); setRecFreq("weekly");
    load();
  };

  // Private iCal feed the host can subscribe to in their calendar app.
  const calendarUrl = calendarToken
    ? `${process.env.NEXT_PUBLIC_SUPABASE_URL || ""}/functions/v1/partner-calendar?token=${calendarToken}`
    : "";
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-6">
        {priced.length === 0 ? (
          ...
          <>
            ...
            {/* Recurring plan creator (weekly OR monthly, pay per clean) */}
            <Card>
              <CardContent className="p-4 space-y-3">
                <h2 className="text-sm font-bold flex items-center gap-1.5"><RiRepeatLine className="w-4 h-4 text-primary" /> Add a recurring plan</h2>
                <p className="text-xs text-muted-foreground">Auto-schedule cleans every week or month. Each clean is charged to your card on file at its per-turnover rate — pay per clean, no subscription.</p>
                <select className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm" value={recProp} onChange={(e) => setRecProp(e.target.value)}>
                  <option value="">Select a property…</option>
                  {priced.map((pp) => <option key={pp.id} value={pp.id}>{pp.nickname || pp.address} — ${Number(pp.turnover_price).toFixed(0)}/clean</option>)}
                </select>
                <div className="flex gap-1.5">
                  {(["weekly", "monthly"] as const).map((f) => (
                    <button key={f} type="button" onClick={() => setRecFreq(f)}
                      className={cn("flex-1 px-3 py-2 rounded-lg text-sm font-medium border capitalize", recFreq === f ? "text-white border-transparent" : "bg-white text-muted-foreground")}
                      style={recFreq === f ? { background: "#5500FF" } : undefined}>{f}</button>
                  ))}
                </div>
                {recFreq === "weekly" ? (
                  <div className="flex flex-wrap gap-1.5">
                    {DOW.map((d) => (
                      <button key={d.v} type="button" onClick={() => setRecDows((p) => p.includes(d.v) ? p.filter((x) => x !== d.v) : [...p, d.v])}
                        className={cn("px-2.5 py-1 rounded-full text-xs font-medium border", recDows.includes(d.v) ? "text-white border-transparent" : "bg-white text-muted-foreground")}
                        style={recDows.includes(d.v) ? { background: "#5500FF" } : undefined}>{d.label}</button>
                    ))}
                  </div>
                ) : (
                  <div>
                    <Label className="text-xs">Day of month</Label>
                    <select className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm" value={recDom} onChange={(e) => setRecDom(e.target.value)}>
                      {Array.from({ length: 31 }, (_, i) => i + 1).map((d) => <option key={d} value={d}>{ORDINAL(d)}</option>)}
                    </select>
                    <p className="text-[11px] text-muted-foreground mt-1">If a month is shorter, we use its last day.</p>
                  </div>
                )}
                <div className="grid grid-cols-2 gap-2">
                  <div><Label className="text-xs">Checkout</Label><Input type="time" value={recWs} onChange={(e) => setRecWs(e.target.value)} /></div>
                  <div><Label className="text-xs">Next check-in by</Label><Input type="time" value={recWe} onChange={(e) => setRecWe(e.target.value)} /></div>
                </div>
                <Button onClick={saveRecurring} disabled={recSaving} className="w-full" style={{ background: "#5500FF" }}>
                  {recSaving ? <RiLoader4Line className="w-4 h-4 animate-spin" /> : "Save recurring plan"}
                </Button>
              </CardContent>
            </Card>
            ...
            {calendarUrl && (
              <Card>
                <CardContent className="p-4 space-y-2">
                  <h2 className="text-sm font-bold flex items-center gap-1.5"><RiCalendarEventLine className="w-4 h-4 text-primary" /> Add cleans to your calendar</h2>
                  <p className="text-xs text-muted-foreground">Subscribe in Google / Apple Calendar to see every scheduled turnover automatically.</p>
                  <div className="flex items-center gap-2">
                    <Input value={calendarUrl} readOnly className="font-mono text-xs" />
                    <Button variant="outline" size="sm" onClick={() => { void navigator.clipboard.writeText(calendarUrl); toast.success("Calendar link copied"); }}>Copy</Button>
                  </div>
                  <a href={calendarUrl.replace(/^https?:/, "webcal:")} className="text-xs font-medium text-[#5500FF] hover:underline">Subscribe now →</a>
                </CardContent>
              </Card>
            )}
                    ...
                    <div className="text-sm min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {rc.frequency === "monthly"
                          ? `Monthly · ${ORDINAL(Number(rc.day_of_month) || 1)}`
