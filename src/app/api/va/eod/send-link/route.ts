...
import { assertAdminIssuableDate, EodError } from "@/lib/va-performance/eod";
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  try {
    ...
    if (action === "send" || action === "rotate") {
      ...
      const requested = String(body.workDate || "") || today;

      // Only an admin may issue a link for a day other than today. The cron
      // path (viaSecret) is the system's own daily send and stays on today.
      const otherDay = requested !== today;
      if (otherDay) {
        if (!admin) {
          return NextResponse.json(
            { ok: false, error: "Only an admin can send an EOD link for another day." },
            { status: 403 },
          );
        }
        assertAdminIssuableDate(requested, settings);
      }

      const result = await sendEodLink(va, {
        workDate: requested,
        issuedBy: admin ? { userId: admin.userId, email: admin.email } : undefined,
        adminIssued: otherDay,
      });
