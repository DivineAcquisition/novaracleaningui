// Branded contractor-statement PDF — same Novara document standard as
// walkthrough / screening / EOD records (violet header, Helvetica, US Letter).
// SERVER ONLY.

import type { QcStatementPdfInput } from "./qc-statement";
...
export async function buildQcStatementPdf(input: QcStatementPdfInput): Promise<Uint8Array> {
  ...
  if (input.attachmentNames.length === 0) {
    qa("Uploads", "None attached.");
  } else {
    qa("Uploads", input.attachmentNames.map((n, i) => `${i + 1}. ${n}`).join("\n"));
  }
  ...
  pages.forEach((p, i) => {
    ...
    p.drawText(`Page ${i + 1} of ${pages.length}`, {
      x: PAGE_W - MARGIN_X - bold.widthOfTextAtSize(`Page ${i + 1} of ${pages.length}`, 7),
      y: 30,
      size: 7,
      font,
      color: gray,
...
/** Save the statement first; PDF failure must not discard it. */
export async function saveThenGeneratePdf<T>(args: {
  save: () => Promise<T>;
  generate: (saved: T) => Promise<void>;
  onPdfFailure: (saved: T, error: unknown) => Promise<void>;
}): Promise<{ saved: T; pdfOk: boolean; pdfError: string | null }> {
  const saved = await args.save();
  try {
    await args.generate(saved);
    return { saved, pdfOk: true, pdfError: null };
  } catch (error) {
    await args.onPdfFailure(saved, error);
    return {
      saved,
      pdfOk: false,
