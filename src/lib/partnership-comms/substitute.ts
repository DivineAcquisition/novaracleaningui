...
/**
 * Substitute `{{var}}` and the Proposals-tab `[Name]` / `[link]` aliases.
 * Unknown placeholders become empty strings so a missing field never leaks
 * braces into a customer-facing message.
 */
export function substitutePartnershipTemplate(
  ...
  return curly.replace(/\[([^\]]+)\]/g, (full, raw: string) => {
    ...
    // Unknown [brackets] that look like copy (not placeholders) stay put.
...
export function partnershipVars(
  vars: Record<string, string | number | null | undefined> | undefined,
): Record<string, string> {
  return stringifyVars(vars);
}
